...
- (CATransform3D)transformForPosition:(CGFloat)position {
    ...
    if (position < - _pageItemSpace * 1.5)
    {
        position += 3 * _pageItemSpace;
    }
    else if (position > _pageItemSpace * 1.5)
    {
        position -= 3 * _pageItemSpace;
    }

    CATransform3D translationTransform = CATransform3DMakeTranslation(position, 0.0, fabs(position / _pageItemSpace) * -1200.0);
