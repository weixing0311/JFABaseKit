...
- (void)dealloc
{
    [self.panGestureRecognizer removeObserver:self forKeyPath:@"enabled"];
...
- (void)viewDidLoad
{
    ...
    [recognizer addObserver:self forKeyPath:@"enabled" options:NSKeyValueObservingOptionNew context:nil];
}

- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    if ([object isKindOfClass:[UIPanGestureRecognizer class]] && [keyPath isEqualToString:@"enabled"])
    {
        NSNumber *enabled = [change objectForKey:NSKeyValueChangeNewKey];
        if (enabled)
        {
            [lastScreenShotView setHidden:! enabled.boolValue];
        }
    }
}
